        ...
        while True: 
           ...
           #          Hour-wise Mobility Indicator Graph                #
            ...
            for d in range(0,z+1):
                self.total_value_count[d] = self.hour_truck_count[d] + self.hour_person_count[d] + self.hour_bus_count[d] + self.hour_bicycle_count[d]
            
            z = z + 1
            print(z)
            print("Length", len(self.hour_car_count))
            print("Length", len(self.hour_car_count)) 
            for c in range(0, z):
               self.plotter[c] = c + 1
            plt.plot(self.plotter, self.total_value_count)
            plt.xlabel('Hour')
            plt.ylabel('Count')
            plt.title("Economic Indicator")
            plt.show(block = False)
            print(self.total_value_count)
         #   print("Length", len(self.hour_car_count))
         #   print("Bicycle count this hour : ", self.hour_bicycle_count)
         #   print("Chair count this hour : ", self.hour_person_count)

         #           Hour-Wise Economic Indicator Graph               #
