...
class LabelWindow(Gtk.Window):
   ...
   #        r = requests.get('http://localhost:8080/counter/areas')
             ...
             area_assign = 1
             if(y is  0):
               sub_value = jsn["counterSummary"][counter_area]["person"]
               truck_count[y] = jsn["counterSummary"][counter_area]["person"]
             else:
               for x in range(1,y-1):
                  sub_value = sub_value + truck_count[x]
               truck_count[y] = jsn["counterSummary"][counter_area]["person"] - sub_value
             print("Truck count this hour is ",truck_count[y])
             print(jsn["counterSummary"][counter_area]["person"])
             print(sub_value)
             y = y+1
             #else:
             #   car_count[y] = jsn["counterSummary"][counter_area]["car"]-car_count[y-1]
            # y = y+1
             print("Car count this hour ",truck_count)
